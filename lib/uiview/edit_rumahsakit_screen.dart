...
class _EditRumahSakitScreenState extends State<EditRumahSakitScreen> {
  ...
  void initState() {
    ...
    tipeController = TextEditingController(text: widget.rumahSakit.tipe);
  ...
  Future<void> updateData() async {
    if (_formKey.currentState!.validate()) {
      final response = await http.put(
        Uri.parse(
          'http://192.168.43.228:8000/api/rumahsakit/${widget.rumahSakit.id}',
        ),
      ...
      if (response.statusCode == 200) {
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(SnackBar(content: Text('✅ Data berhasil di edit')));
        Navigator.pop(context, true);
      } else {
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(SnackBar(content: Text('❌ Gagal mengedit data')));
      }
    }
  }
